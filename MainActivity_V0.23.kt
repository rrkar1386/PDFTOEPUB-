...
data class AnalysisOutput(
    ...
    val visualObjects: List<VisualObject> = emptyList(),
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val title = TextView(this).apply {
            text = "PDF → EPUB\nV0.23 — Protected V0.21 + Page-Aware Visual Objects"
            textSize = 28f
        }
    ...
    private fun analyzePdf(uri: Uri) {
        ...
        Thread {
            try {
                val result = contentResolver.openInputStream(uri)?.use { analyzeDocument(it, uri) }
                    ?: AnalysisOutput(
                        "Could not open PDF.",
                        emptyList()
                    )
                // V0.23: build the figure/graphic specs from the page-aware visual
                // object model first. This is the critical bridge that keeps
                // standalone figures (including figures without usable captions)
                // from disappearing during asset generation.
                val figureSpecs = buildFigureSpecs(result.pages, result.visualObjects)
                val assetFigures = try {
                    materializeFigureAssets(uri, figureSpecs)
                } catch (_: Exception) {
                    figureSpecs
                }
                val visualObjects = result.visualObjects
                val diagnostic = result.text +
                    "\nVISUAL OBJECT RECONSTRUCTION (V0.23)\n" +
                    "===================================\n" +
                    "Visual candidates: ${visualObjects.size}\n" +
                    "Accepted figure/graphic objects: ${visualObjects.count { it.kind != "REJECT" }}\n" +
                    "Figure assets generated: ${assetFigures.count { !it.assetPath.isNullOrBlank() }}\n" +
                    "Figures are page-aware objects and are retained in source page/Y order.\n" +
                    "Caption matching is associative, not a prerequisite for visual-object detection.\n" +
                    "V0.21 text/table classification remains protected.\n"
                runOnUiThread {
    ...
    private fun analyzeDocument(input: InputStream, sourceUri: Uri): AnalysisOutput {
        PDDocument.load(input).use { doc ->
            ...
            val visualObjects = detectVisualObjects(sourceUri, pages)
            return formatResult(doc.numberOfPages, rawGlyphs, cleanedGlyphs, duplicateGlyphs, totalLines, furniture, pages, visualObjects)
        }
    }
...
// -------------------------- V0.23 page-aware visual object model --------------------------

private data class GridBox(
    val left: Int, val top: Int, val right: Int, val bottom: Int, val cells: Int
) {
...
/**
 * V0.23 visual branch. It is intentionally independent of text classification and
 * table detection. Known text blocks are masked from the rendered page, then the
 * remaining substantial visual regions are evaluated as figure/graphic candidates.
 * A caption is associated later; it is never required for visual-object existence.
 */
private fun detectVisualObjects(uri: Uri, pages: List<PageModel>): List<VisualObject> {
    if (pages.isEmpty()) return emptyList()
    val descriptor = contentResolver.openFileDescriptor(uri, "r") ?: return emptyList()
    descriptor.use { pfd ->
        PdfRenderer(pfd).use { renderer ->
            val result = mutableListOf<VisualObject>()
            pages.forEach { pageModel ->
                val pageIndex = pageModel.pageNumber - 1
                if (pageIndex !in 0 until renderer.pageCount) return@forEach
                renderer.openPage(pageIndex).use { page ->
                    val targetWidth = 900
                    val targetHeight = max(1, (targetWidth * page.height.toFloat() / page.width).toInt())
                    ...
                    try {
                        ...
                        val candidates = findVisualCandidateBoxes(bitmap, pageModel, sx, sy)
                        val merged = mergeVisualCandidateBoxes(candidates, targetWidth, targetHeight)
                        merged.forEach { box ->
                            val pdfX = box.left / sx
                            val pdfY = box.top / sy
                            val pdfW = box.width / sx
                            val pdfH = box.height / sy
                            val caption = associateCaption(pageModel, pdfX, pdfY, pdfW, pdfH)
                            val confidence = visualConfidence(box, targetWidth, targetHeight)
                            if (confidence != null) {
                                result += VisualObject(
                                    ...
                                    w = pdfW,
                                    h = pdfH,
                                    kind = if (caption.isNotBlank()) "FIGURE" else "GRAPHIC",
            ...
            return result.sortedWith(compareBy<VisualObject> { it.pageNumber }.thenBy { it.y }.thenBy { it.x })
        }
    }
}

private fun findVisualCandidateBoxes(
    bitmap: Bitmap,
    page: PageModel,
    sx: Float,
    sy: Float
    ...
    val cell = 6
    val gridW = max(1, (bitmap.width + cell - 1) / cell)
    val gridH = max(1, (bitmap.height + cell - 1) / cell)
    val active = BooleanArray(gridW * gridH)
    val textBlocks = page.blocks.filter {
        it.type !in setOf("PAGE_NUMBER", "RUNNING_HEADER", "RUNNING_FOOTER", "INTERNAL_DEBUG")
    }

    fun masked(gx: Int, gy: Int): Boolean {
        val px0 = gx * cell
        val py0 = gy * cell
        val px1 = min(bitmap.width, px0 + cell)
        val py1 = min(bitmap.height, py0 + cell)
        val cx = (px0 + px1) / 2f / sx
        val cy = (py0 + py1) / 2f / sy
        return textBlocks.any {
            cx >= it.x - 2f && cx <= it.x + it.w + 2f &&
                cy >= it.y - 2f && cy <= it.y + it.h + 2f
        }
    }
    ...
    for (gy in 0 until gridH) {
        for (gx in 0 until gridW) {
            ...
            val x0 = gx * cell
            val y0 = gy * cell
            val x1 = min(bitmap.width, x0 + cell)
            val y1 = min(bitmap.height, y0 + cell)
            var nonWhite = 0
            var total = 0
            var variation = 0
            for (y in y0 until y1) {
                for (x in x0 until x1) {
                    ...
                    if (maxC - minC > 18) variation++
                }
            }
            val occupancy = if (total == 0) 0f else nonWhite.toFloat() / total
            val colorRatio = if (total == 0) 0f else variation.toFloat() / total
            active[gy * gridW + gx] = occupancy >= 0.10f || colorRatio >= 0.08f
        }
    }

    // Close small gaps inside diagrams/photos but keep ordinary separated text lines apart.
    val closed = BooleanArray(active.size)
    for (gy in 0 until gridH) for (gx in 0 until gridW) {
        var count = 0
        for (dy in -1..1) for (dx in -1..1) {
            val nx = gx + dx; val ny = gy + dy
            if (nx in 0 until gridW && ny in 0 until gridH && active[ny * gridW + nx]) count++
        }
        closed[gy * gridW + gx] = active[gy * gridW + gx] || count >= 5
    }
    ...
    for (start in closed.indices) {
        ...
        var minX = gridW; var minY = gridH; var maxX = 0; var maxY = 0; var count = 0
        while (head < tail) {
            val idx = queue[head++]; val gx = idx % gridW; val gy = idx / gridW
            minX = min(minX, gx); minY = min(minY, gy); maxX = max(maxX, gx + 1); maxY = max(maxY, gy + 1); count++
            for (dy in -1..1) for (dx in -1..1) {
                if (dx == 0 && dy == 0) continue
                val nx = gx + dx; val ny = gy + dy
                if (nx !in 0 until gridW || ny !in 0 until gridH) continue
                val ni = ny * gridW + nx
                if (closed[ni] && !visited[ni]) { visited[ni] = true; queue[tail++] = ni }
            }
        }
        if (count >= 18) {
            boxes += GridBox(minX * cell, minY * cell, min(bitmap.width, maxX * cell), min(bitmap.height, maxY * cell), count)
        }
    }
    return boxes
}

private fun mergeVisualCandidateBoxes(boxes: List<GridBox>, width: Int, height: Int): List<GridBox> {
    if (boxes.isEmpty()) return emptyList()
    val filtered = boxes.filter {
        val area = it.width.toFloat() * it.height / (width * height).toFloat()
        it.width >= width * 0.12f && it.height >= height * 0.045f && area >= 0.0025f &&
            it.top >= height * 0.08f && it.bottom <= height * 0.92f
    }.toMutableList()
    ...
    while (changed) {
        changed = false
        outer@ for (i in 0 until filtered.size) {
            for (j in i + 1 until filtered.size) {
                val a = filtered[i]; val b = filtered[j]
                val gapX = horizontalDistance(a.left, a.right, b.left, b.right)
                val gapY = verticalDistance(a.top, a.bottom, b.top, b.bottom)
                val verticalOverlap = overlap(a.top.toFloat(), a.bottom.toFloat(), b.top.toFloat(), b.bottom.toFloat())
                val horizontalOverlap = overlap(a.left.toFloat(), a.right.toFloat(), b.left.toFloat(), b.right.toFloat())
                val nearPanels = (horizontalOverlap > min(a.width, b.width) * 0.25f && gapY <= height * 0.025f) ||
                    (verticalOverlap > min(a.height, b.height) * 0.25f && gapX <= width * 0.025f)
                if (nearPanels) {
                    filtered[i] = GridBox(min(a.left, b.left), min(a.top, b.top), max(a.right, b.right), max(a.bottom, b.bottom), a.cells + b.cells)
                    filtered.removeAt(j)
                    changed = true
                    break@outer
                }
    ...
    return filtered.sortedWith(compareBy<GridBox> { it.top }.thenBy { it.left })
}

private fun visualConfidence(box: GridBox, width: Int, height: Int): String? {
    val areaRatio = box.width.toFloat() * box.height / (width * height).toFloat()
    val sizeScore = when {
        areaRatio >= 0.12f -> 2
        areaRatio >= 0.04f -> 1
        else -> 0
    }
    val shapeScore = if (box.width >= width * 0.20f || box.height >= height * 0.12f) 1 else 0
    return when (sizeScore + shapeScore) {
        3 -> "HIGH"
        2 -> "MEDIUM"
        else -> null
    }
...
private fun associateCaption(page: PageModel, x: Float, y: Float, w: Float, h: Float): String {
    val candidates = page.blocks.filter { it.type == "FIGURE_CAPTION" }.mapNotNull { c ->
        val horizontal = overlap(x, x + w, c.x, c.x + c.w)
        val belowGap = c.y - (y + h)
        val aboveGap = y - (c.y + c.h)
        val gap = when {
            belowGap >= -4f -> belowGap
            aboveGap >= -4f -> aboveGap
            else -> Float.MAX_VALUE
        }
        if (horizontal <= 0f || gap > max(90f, page.height * 0.12f)) null
        else Pair(gap, c.text)
    }
    return candidates.minByOrNull { it.first }?.second ?: ""
}

private fun buildFigureSpecs(pages: List<PageModel>, visualObjects: List<VisualObject>): List<FigurePreviewSpec> {
    val specs = mutableListOf<FigurePreviewSpec>()
    val visual = visualObjects.filter { it.kind == "FIGURE" || it.kind == "GRAPHIC" }
    visual.forEachIndexed { index, v ->
        specs += FigurePreviewSpec(
            ...
            caption = v.caption,
            detection = when {
                v.caption.isBlank() && v.kind == "GRAPHIC" -> "VISUAL_GRAPHIC"
                v.caption.isBlank() -> "VISUAL_ONLY"
                else -> "VISUAL_CAPTION_ASSOCIATED"
            }
        )
    }
    // Preserve V0.21 caption detection as a fallback only when no visual object covers it.
    val existing = collectFigurePreviewSpecs(pages)
    existing.forEach { e ->
        val covered = specs.any { s ->
            s.pageNumber == e.pageNumber &&
                overlap(s.x, s.x + s.w, e.x, e.x + e.w) > min(s.w, e.w) * 0.25f &&
                overlap(s.y, s.y + s.h, e.y, e.y + e.h) > min(s.h, e.h) * 0.20f
        }
        if (!covered) specs += e
    }
    return specs.sortedWith(compareBy<FigurePreviewSpec> { it.pageNumber }.thenBy { it.y }.thenBy { it.x })
}
...
private fun verticalDistance(aTop: Int, aBottom: Int, bTop: Int, bBottom: Int): Float {
    return when {
        aBottom < bTop -> (bTop - aBottom).toFloat()
        bBottom < aTop -> (aTop - bBottom).toFloat()
        else -> 0f
    }
}
...
/**
 * V0.23: materialize the already-detected visual figure regions as standalone
...
private fun materializeFigureAssets(
    ...
    val assetDir = java.io.File(filesDir, "figure_assets_v023").apply { mkdirs() }
...
// -------------------------- diagnostics --------------------------
    ...
    /**
     * V0.23 diagnostic representation of the actual document object stream.
     * Figures are inserted at source-page/Y position rather than appended after
     * all text. The EPUB writer can consume the same ordered object model later.
     */
    private fun appendInterleavedDocumentObjects(
        sb: StringBuilder,
        pages: List<PageModel>,
        visualObjects: List<VisualObject>
    ) {
        pages.forEach { page ->
            val pageFigures = visualObjects.filter { it.pageNumber == page.pageNumber }
            val blocks = pageReadingOrder(page)
                .filterNot { isGeneratedQuestionSourceHeading(it.text) }

            val emitted = mutableSetOf<Int>()
            blocks.forEachIndexed { index, block ->
                pageFigures.filterIndexed { i, _ ->
                    i !in emitted && itY(pageFigures[i]) <= block.y
                }.forEachIndexed { _, figure ->
                    val fi = pageFigures.indexOf(figure)
                    if (fi >= 0 && emitted.add(fi)) {
                        sb.append("[FIGURE ${figure.confidence} — page ${figure.pageNumber} y=${fmt(figure.y)}]")
                        if (figure.caption.isNotBlank()) sb.append(" ${figure.caption}")
                        sb.append("\n")
                    }
                }
                when (block.type) {
                ...
                if (index == blocks.lastIndex) {
                    pageFigures.forEachIndexed { fi, figure ->
                        if (fi !in emitted) {
                            emitted.add(fi)
                            sb.append("[FIGURE ${figure.confidence} — page ${figure.pageNumber} y=${fmt(figure.y)}]")
                            if (figure.caption.isNotBlank()) sb.append(" ${figure.caption}")
    ...
    private fun itY(figure: VisualObject): Float = figure.y
    ...
    private fun formatResult(
        ...
        sb.append("Visual objects accepted: ${visualObjects.count { it.kind != "REJECT" }}\n")
        sb.append("Visual objects with captions: ${visualObjects.count { it.caption.isNotBlank() }}\n")
        ...
        appendInterleavedDocumentObjects(sb, pages, visualObjects)

        sb.append("\nVISUAL OBJECT READING ORDER\n")
        sb.append("===========================\n")
        if (visualObjects.isEmpty()) sb.append("No accepted visual objects detected.\n")
        visualObjects.sortedWith(compareBy<VisualObject> { it.pageNumber }.thenBy { it.y }.thenBy { it.x }).forEach { v ->
            sb.append("PAGE ${v.pageNumber}: ${v.kind} ${v.confidence} x=${fmt(v.x)} y=${fmt(v.y)} w=${fmt(v.w)} h=${fmt(v.h)}\n")
            if (v.caption.isNotBlank()) sb.append("CAPTION: ${v.caption}\n")
        }
        ...
        val figures = buildFigureSpecs(pages, visualObjects)
