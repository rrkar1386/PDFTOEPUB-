...
data class AnalysisOutput(
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val title = TextView(this).apply {
            text = "PDF → EPUB\nV0.21 — Protected V0.20 + Visual Figure Detection"
            textSize = 28f
        }
        ...
        Thread {
            try {
                ...
                val diagnostic = result.text +
                    "\nVISUAL FIGURE DETECTION (V0.21)\n" +
                    "==============================\n" +
                    "Caption-associated figure regions: ${upgradedFigures.size}\n" +
                    "Raster/vector page rendering is used only for figure verification; V0.20 text, table,\n" +
                    "reading-order and structural classification paths are unchanged.\n"
                runOnUiThread {
                    output.text = diagnostic
                    showFigurePreviews(uri, upgradedFigures)
                }
            } catch (e: Exception) {
